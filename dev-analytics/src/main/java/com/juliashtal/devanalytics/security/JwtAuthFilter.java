...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    private boolean isTokenVersionValid(String jwt, UserDetails userDetails) {
        ...
        boolean valid = tokenVersion != null && tokenVersion == custom.getUser().getTokenVersion();
        if (!valid) {
            log.warn("JWT token version mismatch for user: {} (token version={}, current={})",
                    custom.getUsername(), tokenVersion, custom.getUser().getTokenVersion());
        }
        return valid;
    }
