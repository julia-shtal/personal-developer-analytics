...
public class DataSourceRepoController {
    ...
    @Operation(summary = "List repositories visible to the stored token (GitHub or GitLab), annotated with alreadyAttached. Cached 60 s.")
    public ResponseEntity<List<DiscoveredRepoDto>> discoverRepos(@PathVariable Long id) {
        Long userId = SecurityUtils.getCurrentUserId();
        DataSourceType type = dataSourceService.getDataSource(id).getType();
        DiscoveryResult result = switch (type) {
            case GITHUB -> gitHubRepositoryService.discoverRepos(userId, id);
            case GITLAB -> gitLabRepositoryService.discoverRepos(userId, id);
            default -> throw new BadRequestException("Only GITHUB and GITLAB datasources support repo discovery");
        };
